...
- 本地库有**中文片源**(豆瓣离线种子脚本导入 `importDoubanSeed.js` + TMDB 批量同步 `syncTmdbMovies.js`,搜索/AI 触达的新片还会经 `movieUpsert` **幂等回写**沉淀为本地资产),体验稳、零外部依赖;但用户可能输**英文原名**,本地 LIKE 必然 0 命中。
...
四把钥匙全齐才触发([movie.js:220-225](server/router_handler/movie.js#L220-L225)):
```
useHybrid && page === 1 && q 非空 && 本地第一页条数 < Math.min(pageSize, HYBRID_MIN_LOCAL_RESULTS)
```
1. **① 开关 `useHybrid`(URL 带 `hybrid=1`)**:总闸。TMDB 挂掉 / 没配 token 时前端不带 hybrid,安全退化为纯本地。
2. **② 只在第 1 页**:第一页是"这词本地到底行不行"的相关性裁判——第一页满屏 = 本地够;用户翻到第 2 页说明已接受本地结果,再每页外呼又慢又让总页数飘;fallback 只在第 1 页合并一次,分页语义稳定。
3. **③ `q` 非空**:首页默认列表 / 分类浏览不带关键词,绝不外呼——没有词可搜,白白消耗外部配额。
4. **④ 不足一屏才补(不是"0 条才兜底")**:搜到 0 条 和 只中 2 条,对用户观感同样单薄;阈值是"第一页凑不满一屏就找 TMDB 捞"。`Math.min(pageSize, 5)` 兼容用户调小每页条数:

| pageSize(默认 10) | 触发线(第一页本地条数) |
|---|---|
| 10 | < 5 |
| 3 | < 3 |
| 20 | < 5 |

一句话:**"第一页没被本地结果填满,就去 TMDB。本地够用就别打扰外部,不够才补,且只在第 1 页补一次。"**

**走查一遍**:搜《盗梦空间》→ 本地命中 8 条、第一页满屏 → 条件④不满足 → 纯本地秒回,不碰外部;搜 "Inception" → 本地 0 条 → 四钥匙全齐 → TMDB 搜索 → `movieUpsert` 幂等落库 → 按本地 id 合并返回 → 下次再搜 "Inception" 直接本地命中(落库=沉淀资产)。

- 可谈点:阈值做成常量 `HYBRID_MIN_LOCAL_RESULTS`、可调,是"策略参数化"的简单实践。
